...
class CarliniL2_specific:
    def __init__(self, sess, model, batch_size=1, confidence=CONFIDENCE,
                 targeted=TARGETED, learning_rate=LEARNING_RATE,
                 binary_search_steps=BINARY_SEARCH_STEPS, max_iterations=MAX_ITERATIONS,
                 abort_early=ABORT_EARLY,
                 initial_const=INITIAL_CONST,
                 boxmin=-0.5, boxmax=0.5, extra_loss=True,e_kernel_train=None,e_median=None,sigma2=1.0/0.26):
        """
        ...
        self.kernel_train = tf.Variable(np.zeros((100,64)), dtype=tf.float32)  ##############64 is specifically for resnet
        ...
        self.assign_kernel_train = tf.placeholder(tf.float32, [100,64])##############64 is specifically for resnet
        ...
        self.tsne_logit_adv = tf.reshape(tsne_logit_adv,[1,64])
        ...
        if extra_loss == True:
            self.e_kernel_train = e_kernel_train # 1000X64X10
            self.e_median = e_median  # 10X1
            self.sigma2 = sigma2
            self.log_density_ratio = -tf.log(1e-30+tf.reduce_mean(tf.exp(-tf.reduce_sum(tf.square(tf.tile(self.tsne_logit_adv,[100,1])
                                            -self.kernel_train),axis=1)/self.sigma2),axis=0))+tf.log(self.median)
            #self.log_density_ratio = tf.matmul(m_tsne_logits_adv, inter_mat_adv,transpose_a=True)+self.median
        ...
        self.setup.append(self.kernel_train.assign(self.assign_kernel_train))
    ...
    def attack_batch(self, origs, imgs, labs, pre_labs):
        ...
        def compare(x, y):
            ...
            if self.TARGETED:
                ...
                return x != y
        # num_eps = 1e-2
        # con_nor = np.max(pre_labs[:batch_size])
        # def compare_confidence(x, y):
        #     if not isinstance(x, (float, int, np.int64)):
        #         x_lab = np.argmax(x)
        #         con = np.max(x)
        #     else:
        #         print('Error compare_confidence!!!')
        #     if self.TARGETED:
        #         return (x_lab == y and con > (con_nor-num_eps))
        #     else:
        #         return (x_lab != y and con > (con_nor-num_eps))
        ...
        o_bestl2 = [1e20] * batch_size
        #o_bestscore = [-1] * batch_size
        o_bestattack = [np.zeros(imgs[0].shape)] * batch_size
        o_bestattack_on = [0] * batch_size
        o_log_density_ratio = [1e20] * batch_size

        #record best extra attack if extra > 0
        o_bestextra = [1e20] * batch_size
        o_bestextraattack = [np.zeros(imgs[0].shape)] * batch_size
        o_log_density_ratio_extra = [1e20] * batch_size

        o_bestattack_final = [np.zeros(imgs[0].shape)] * batch_size
        o_log_density_ratio_final = [1e20] * batch_size
        ...
        for outer_step in range(self.BINARY_SEARCH_STEPS):
            ...
            self.sess.run(self.setup, {self.assign_timg: batch,
                                       ...
                                       self.assign_kernel_train: np.reshape(self.e_kernel_train[:,:,s_index],(100,64)),
            ...
            for iteration in range(self.MAX_ITERATIONS):
                ...
                if self.ABORT_EARLY and iteration>1 and iteration % (self.MAX_ITERATIONS // 10) == 0:
                    if l > prev * .9999:
                        break
                    prev = l
                ...
                for e, (l2, sc, ii) in enumerate(zip(l2s, scores, nimg)):
                    ...
                    if compare(sc, np.argmax(batchlab[e])) and extra[e] <= o_bestextra[e]:
                        o_bestextra[e] = extra[e]
                        o_bestextraattack[e] = ii
                        o_log_density_ratio_extra[e] = log_density_ratio[e]
            ...
            for e in range(batch_size):
                if compare(bestscore[e], np.argmax(batchlab[e])) and bestscore[e] != -1:
                    ...
                    if upper_bound[e] < 1e9:
                        ...
                        CONST[e] *= 10
        for e in range(batch_size):
            if o_bestattack_on[e] == 1:
                o_bestattack_final[e] = o_bestattack[e]
                o_log_density_ratio_final[e] = o_log_density_ratio[e]
            else:
                o_bestattack_final[e] = o_bestextraattack[e]
                o_log_density_ratio_final[e] = o_log_density_ratio_extra[e]

        # return the best solution found
        o_bestl2 = np.array(o_bestl2)
        return o_bestattack_final,o_bestattack_on,o_log_density_ratio_final