...
def jsma(model, x, hps, RCE_train, y, epochs=1.0, eps=1.0, clip_min=0.0, clip_max=1.0,
    ...
    def _fn(i):
        ...
        xadv = _jsma_fn(model, xi, hps, RCE_train, yi, epochs=epochs, eps=eps,
                        clip_min=clip_min, clip_max=clip_max,
                        min_proba=min_proba)
        return xadv[0]

    return tf.map_fn(_fn, tf.range(n), dtype=tf.float32, back_prop=False,name='jsma_batch')


def _jsma_impl(model, xi, hps, RCE_train, yi, epochs, eps=1.0, clip_min=0.0, clip_max=1.0,
               min_proba=0.0):

    def _cond(x_adv, epoch, pixel_mask):
        ybar = tf.reshape(model(hps, x_adv, RCE_train, logits=False), [-1])
        proba = ybar[yi]
    ...
    def _body(x_adv, epoch, pixel_mask):
        ybar = model(hps, x_adv, RCE_train, logits=False)
...
def _jsma2_impl(model, xi, hps, RCE_train, yi, epochs, eps=1.0, clip_min=0.0, clip_max=1.0,
               min_proba=0.0):
